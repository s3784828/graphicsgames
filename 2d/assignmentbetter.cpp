...
Vector2 calculateR0(float hyp, float xInc, float yInc, float s)
{
  ...
  float _x = xInc * cosf(toRad(bAngle)) - yInc * sinf(toRad(bAngle));
  float _y = xInc * sinf(toRad(bAngle)) + yInc * cosf(toRad(bAngle));
 
  float aAngle = pCoord.angle;

  float r0x = (hyp) * cosf(toRad(aAngle + bAngle)) + _x;
  float r0y = (hyp) * sinf(toRad(aAngle + bAngle)) + _y;
